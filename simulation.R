...
try(if(!file.exists("INTERNAL_JUMP_ENGINE_FILE")) stop("PERL file not found at current directory"))
try(if(!file.exists("INTERNAL_FILE_2")) stop("TrendJumpDetect.pm file not found at current directory"))
...
jam <- function(filename){
  prog="INTERNAL_JUMP_ENGINE_FILE"
